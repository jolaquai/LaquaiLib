...
public static class StringExtensions
{
    ...
    /// <param name="end">The zero-based <see cref="List{T}"/> index at which the range ends.</param>
    /// <returns>A shallow copy of a range of elements in the source <see cref="List{T}"/>.</returns>
    public static List<T> RangeFrom<T>(this List<T> source, int start, int end) => source.GetRange(start, end - start + 1);
