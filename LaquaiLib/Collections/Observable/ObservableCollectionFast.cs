...
public class ObservableCollectionFast<T> : INotifyCollectionChanged, ICollection<T>, IEnumerable<T>
{
    ...
    /// <summary>
    /// Gets the backing store of the <see cref="ObservableCollection{T}"/>.
    /// </summary>
    ...
    /// <summary>
    /// A <see cref="Func{T, TResult}"/> that determines whether an element should be included when enumerating the collection.
    /// </summary>
    ...
    /// </summary>
    public void SortSilent() => items = [.. items.OrderBy(static x => x, Comparer ?? Comparer<T>.Default)];
    #endregion
    ...
    /// <param name="index">The zero-based index at which the <paramref name="item"/> should be inserted.</param>
    /// <param name="item">The object to insert.</param>
    public void InsertSilent(int index, T item) => items.Insert(index, item);
    /// <summary>
    ...
    public void InsertRangeSilent(int index, params ReadOnlySpan<T> items) => this.items.InsertRange(index, items);
    /// <summary>
    ...
    /// </summary>
    public void ClearSilent() => items.Clear();
    /// <inheritdoc/>
    public void Clear()
    {
        ClearSilent();
    ...
    /// </summary>
    /// <param name="item">The item to remove from this <see cref="ObservableCollection{T}"/>.</param>
    public bool RemoveSilent(T item) => items.Remove(item);
    /// <summary>
