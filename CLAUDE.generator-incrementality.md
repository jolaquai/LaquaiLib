...
| 3.5 | `EnumExpanderGenerator` effective accessibility + skip un-emittable enums | DONE | Found at 5.1: reviving the generator at 3.1 broke the real `multitarget/LaquaiLib` build with 10 errors. `CreateModel` took accessibility from `decl.Modifiers` alone, so a `public` enum inside an `internal` type emitted a `public` Data class (CS0053), and private/protected nested enums emitted classes that could not name them (CS0122). Enums nested in a generic type produced an FQN carrying the containing type parameters, unbindable at namespace scope (CS0246). Fixed by walking the `ContainingType` chain: skip if any is generic, take the minimum `DeclaredAccessibility`, skip anything below `internal`. 3 new emission tests |
...
| 3.7 | Auto-generated header + obsolete suppression on enum output | DONE | Found at 5.1. The enum file emitted no `SourceEmitHelper.GeneratedFileHeader` at all, and materializing every member into the arrays surfaced consumers' `[Obsolete]` members as CS0612/CS0618. Both emitted once, inside the write-once block, so `EmptyEnumIsSkippedEntirely` still sees empty output |
