...
public static partial class ArrayExtensions
{
    extension(Array source)
    {
        ...
        public void MultiDimConstrainedCopyTo<T>(int sourceIndex, Array destination, int destinationIndex, int length)
        {
            ...
            var destCopy = Unsafe.As<Array>(destination.Clone());
        ...
        public bool TryGetSpan<T>(out ISpanProvider<T> spanProvider, out Span<T> span)
        {
            if (source is T[] linear)
            {
                span = linear;
                spanProvider = null;
                return true;
            ...
            try
            {
                spanProvider = source.GetSpanProvider<T>();
        ...
        /// <returns><see langword="true"/> if the span could be retrieved, otherwise <see langword="false"/>.</returns>
        public bool TryGetReadOnlySpan<T>(out IReadOnlySpanProvider<T> rosProvider, out ReadOnlySpan<T> span)
        {
            if (source is T[] linear)
            {
                span = linear;
            ...
            try
            {
                rosProvider = source.GetReadOnlySpanProvider<T>();
        ...
        public bool SequenceEqual<T>(Array other, IEqualityComparer<T> equalityComparer = null)
        {
            ...
            try
            {
                _ = source.TryGetReadOnlySpan(out firstSpanProv, out var firstSpan);
