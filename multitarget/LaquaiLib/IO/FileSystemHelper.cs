...
public static partial class FileSystemHelper
{
    ...
    /// <summary>
    /// Constructs a <see cref="DirectoryMap"/> for the specified directory and its immediate children, allowing efficient access to child file system objects with no further I/O operations.
    /// </summary>
    /// <param name="di">The <see cref="DirectoryInfo"/> instance that identifies the directory to map.</param>
    ...
    /// <returns>The constructed <see cref="DirectoryMap"/> instance.</returns>
    /// <remarks>
    ...
    /// </remarks>
    public static DirectoryMap MapDirectory(DirectoryInfo di, bool deep = false) => new DirectoryMap(di, deep);
}

/// <summary>
/// Represents a mapping of a directory's children (files and subdirectories) to their respective <see cref="FileSystemInfo"/> instances, allowing efficient access without further I/O operations.
/// </summary>
public readonly struct DirectoryMap : IEquatable<DirectoryMap>
{
    private static readonly Dictionary<string, FileSystemInfo> _empty = new Dictionary<string, FileSystemInfo>(StringComparer.Ordinal);

    private readonly string _target;
    private readonly Dictionary<string, FileSystemInfo> _fsis;

    /// <summary>
    /// Initializes a new <see cref="DirectoryMap"/> for the directory represented by <paramref name="di"/>.
    /// </summary>
    /// <param name="di">A <see cref="DirectoryInfo"/> instance that identifies the directory to map.</param>
    /// <param name="deep">Whether to map the entire directory structure recursively. This may take a significant amount of time and memory for large directory structures. The default is <see langword="false"/>.</param>
    /// <remarks>
    /// When <paramref name="deep"/> is <see langword="true"/>, FSOs in subdirectories can be accessed using the current OS's default syntax for relative paths.
    /// </remarks>
    public DirectoryMap(DirectoryInfo di, bool deep = false)
    {
        ...
        _fsis = new Dictionary<string, FileSystemInfo>(StringComparer.Ordinal);

        if (deep)
        {
            var currentFullName = di.FullName;
            foreach (var fsi in di.EnumerateFileSystemInfos("*", SearchOption.AllDirectories))
            {
                var relativePath = Path.GetRelativePath(currentFullName, fsi.FullName);
                var added = _fsis.TryAdd(relativePath, fsi);
                Debug.Assert(added, "Duplicate file system object name.");
            }
        }
        else
        {
            ...
            {
                var added = _fsis.TryAdd(fsi.Name, fsi);
                Debug.Assert(added, "Duplicate file system object name.");
            }
        }
    }
    /// <summary>
    /// Initializes a new <see cref="DirectoryMap"/> for the directory at the specified <paramref name="fullPath"/>.
    /// </summary>
    /// <param name="fullPath">The full path to the directory to map.</param>
    /// <param name="deep">Whether to map the entire directory structure recursively. This may take a significant amount of time and memory for large directory structures. The default is <see langword="false"/>.</param>
    /// <remarks>
    /// When <paramref name="deep"/> is <see langword="true"/>, FSOs in subdirectories can be accessed using the current OS's default syntax for relative paths.
    /// </remarks>
    public DirectoryMap(string fullPath, bool deep = false) : this(new DirectoryInfo(fullPath), deep) { }

    /// <summary>
    /// Attempts to get a <see cref="FileSystemInfo"/> instance for the file system object (file or subdirectory) identified by <paramref name="name"/> in the mapped directory.
    /// </summary>
    /// <param name="name">The name of the file system object to get.</param>
    /// <returns>A <see cref="FileSystemInfo"/> instance for the file system object identified by <paramref name="name"/>, or <see langword="null"/> if no such file system object exists in the mapped directory.</returns>
    public readonly FileSystemInfo this[string name] => _fsis.TryGetValue(name, out var fsi) ? fsi : null;
    /// <summary>
    /// Attempts to get a <see cref="FileInfo"/> instance for the file identified by <paramref name="name"/> in the mapped directory.
    /// </summary>
    /// <param name="name">The name of the file to get.</param>
    /// <returns>A <see cref="FileInfo"/> instance for the file identified by <paramref name="name"/>, or <see langword="null"/> if no such file exists in the mapped directory.</returns>
    public readonly FileInfo File(string name) => this[name] as FileInfo;
    /// <summary>
    /// Attempts to get a <see cref="DirectoryInfo"/> instance for the subdirectory identified by <paramref name="name"/> in the mapped directory.
    /// </summary>
    /// <param name="name">The name of the subdirectory to get.</param>
    /// <returns>A <see cref="DirectoryInfo"/> instance for the subdirectory identified by <paramref name="name"/>, or <see langword="null"/> if no such subdirectory exists in the mapped directory.</returns>
    public readonly DirectoryInfo Directory(string name) => this[name] as DirectoryInfo;

    /// <summary>
    /// Determines whether the current <see cref="DirectoryMap"/> is equal to another <see cref="DirectoryMap"/> instance.
    /// </summary>
    /// <param name="other">The <see cref="DirectoryMap"/> instance to compare to.</param>
    /// <returns><see langword="true"/> if the two instances are equal (that is, whether they target the same top-level directory and their map contents are exactly equal), otherwise <see langword="false"/>.</returns>
    public readonly bool Equals(DirectoryMap other)
    {
        if (_target != other._target)
        {
            return false;
        }
        if (ReferenceEquals(_fsis, other._fsis))
        {
            return true;
        }
        if (_fsis.Count != other._fsis.Count)
        {
            return false;
        }

        foreach (var kvp in _fsis)
        {
            if (!other._fsis.TryGetValue(kvp.Key, out var otherFsi) ||
                !string.Equals(kvp.Value.FullName, otherFsi.FullName, StringComparison.Ordinal))
            {
                return false;
            }
        }
    ...
    /// <summary>
    /// Determines whether the current <see cref="DirectoryMap"/> is equal to another object.
    /// </summary>
    /// <param name="obj">The object to compare to.</param>
    /// <returns><see langword="true"/> if <paramref name="obj"/> is a <see cref="DirectoryMap"/> instance and the two instances are equal, otherwise <see langword="false"/>.</returns>
    public override readonly bool Equals(object obj) => obj is DirectoryMap other && Equals(other);

    /// <summary>
    /// Gets a hash code for the current <see cref="DirectoryMap"/> instance.
    /// Will never change for the same instance.
    /// </summary>
    /// <returns>The hash code for the current instance.</returns>
    public override readonly int GetHashCode() => HashCode.Combine(_target, _fsis);
    /// <inheritdoc cref="Equals(DirectoryMap)"/>
    public static bool operator ==(DirectoryMap left, DirectoryMap right) => left.Equals(right);
    /// <summary>
    /// Determines whether two <see cref="DirectoryMap"/> instances are not equal.
    /// </summary>
    /// <param name="left">The first <see cref="DirectoryMap"/> instance to compare.</param>
    /// <param name="right">The second <see cref="DirectoryMap"/> instance to compare.</param>
    /// <returns><see langword="true"/> if the two instances are not equal, otherwise <see langword="false"/>.</returns>
    public static bool operator !=(DirectoryMap left, DirectoryMap right) => !(left == right);
}