...
public static class DispatcherObjectExtensions
{
    extension(DispatcherObject dispatcherObject)
    {
        ...
        public void Dispatch(Action action) => dispatcherObject.Dispatcher.Invoke(action);
        /// <summary>
        ...
        public void Dispatch(Delegate method, params ReadOnlySpan<object> arguments) => dispatcherObject.Dispatcher.Invoke(method, arguments.ToArray());
        /// <summary>
        ...
        public T Dispatch<T>(Func<T> func) => dispatcherObject.Dispatcher.Invoke(func);
        /// <summary>
        ...
        public async Task DispatchAsync(Action action) => await dispatcherObject.Dispatcher.InvokeAsync(action).Task.ConfigureAwait(false);
        /// <summary>
        ...
        public async Task DispatchAsync(Func<Task> asyncDelegate) => await (await dispatcherObject.Dispatcher.InvokeAsync(asyncDelegate).Task.ConfigureAwait(false)).ConfigureAwait(false);
        /// <summary>
        /// Asynchronously invokes the specified <paramref name="asyncDelegate"/> on the thread the <paramref name="dispatcherObject"/> was created on.
        /// </summary>
        ...
        /// <param name="asyncDelegate">The <see cref="Func{TResult}"/> to invoke.</param>
        /// <returns>The <see cref="Task{TResult}"/> returned by the <paramref name="asyncDelegate"/>.</returns>
        public async Task<T> DispatchAsync<T>(Func<Task<T>> asyncDelegate) => await (await dispatcherObject.Dispatcher.InvokeAsync(asyncDelegate).Task.ConfigureAwait(false)).ConfigureAwait(false);
    }
}
